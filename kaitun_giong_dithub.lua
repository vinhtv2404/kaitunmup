...
TextLabel.Text = "TienVinh Hub Kaitun / Cuttay"
...
TextLabel1.Text = "TienVinh Script • .gg/yF4VEq7aDH"
...
script_time = os.time()
if not isfolder(".TienVinh/BloxFruit/Kaitun/") then
               makefolder(".TienVinh/BloxFruit/Kaitun/")
end
   ...
   local badtimedicac = os.time()
         if not isfile(".TienVinh/BloxFruit/Kaitun/eslapedtime."..game.Players.LocalPlayer.Name) then
        
         writefile(".TienVinh/BloxFruit/Kaitun/eslapedtime."..game.Players.LocalPlayer.Name, "0")
         esltime = 0 
         end 
   ctent2 = readfile(".TienVinh/BloxFruit/Kaitun/eslapedtime."..game.Players.LocalPlayer.Name)
   function updatetime()
      if not isfile(".TienVinh/BloxFruit/Kaitun/eslapedtime."..game.Players.LocalPlayer.Name) then
         ...
         ctent = readfile(".TienVinh/BloxFruit/Kaitun/eslapedtime."..game.Players.LocalPlayer.Name)
         if tonumber(ctent) or ctent == "0" or ctent == "1" then
            esltime = tonumber(ctent2 or 0) + (os.time()-(badtimedicac or os.time()))
            writefile(".TienVinh/BloxFruit/Kaitun/eslapedtime."..game.Players.LocalPlayer.Name, tostring(esltime))
...
local Window = Library:CreateWindow({
    ...
    Title = 'TienVinh Community Free Scripts '..os.date("%d".." ".."%B".." ".."%Y"),
...
Info:AddLabel("TienVinh Community Kaitun Script") 
 ...
 local decalsyeeted = true
  ...
  --          game.ReplicatedStorage.Effect.Container.Respawn:Destroy()
         ...
         function SaveSettings2()
            local HttpService = game:GetService("HttpService")
            if not isfolder(".TienVinh/utils/") then
               makefolder(".TienVinh/utils/")
            end
            writefile(".TienVinh/utils/" .. bN, HttpService:JSONEncode(bM))
         end
         function ReadSetting2()
            ...
            function()
               ...
               return HttpService:JSONDecode(readfile(".TienVinh/utils/" .. bN))
            end
            )
            if s then
               return o
...
spawn(function() 
    ...
    warn(109283737)
      if Rapid.DEFAULT_DATA.POBqdNKxAW ~= fetch_data().POBqdNKxAW then
        ...
        bingu("lTienVinh Hub / Server Crashed", "An Kick Request Requested From We Server") 
      end 
    end 
  end)
    ...
    while (fe and wait()) or task.wait(c().Misc.Webhook["Send While"].Delay) do 
      ...
      if c().Misc.Webhook["Send While"]["Account Status"] then 
        ...
        ditmemememememmeme["embeds"] = {{
          ...
          ["footer"] = {
             ["text"] = "TienVinh Cuttay / https://discord.gg/yF4VEq7aDH / "..disp_time(esltime) or "",
           },
